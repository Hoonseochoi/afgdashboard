...
function ContinuousRun12Card({
  ...
  const baseProgress = Math.min(100, (baseFeb / 100000) * 100);
  const extraProgress = Math.min(100, (extraFeb / 100000) * 100);
  ...
  return (
    <div className="relative rounded-2xl border p-5 flex flex-col gap-4 overflow-hidden bg-gradient-to-br from-violet-50 to-violet-100/80 dark:from-violet-950/40 dark:to-violet-900/30 border-violet-200/80 dark:border-violet-700/60 shadow-lg shadow-violet-200/20 dark:shadow-violet-900/20">
      <div className="absolute top-0 left-0 right-0 h-px opacity-60 bg-gradient-to-r from-transparent via-violet-300 to-transparent dark:via-violet-600" />
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-gray-600 dark:text-gray-400">
            1월 구간 + 2월 구간 실적을 한 번에 확인하고, 예상 시상금을 비교합니다.
          </p>
        </div>
        <div className="flex flex-col items-end gap-1">
          ...
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-white/70 dark:bg-gray-800/70 text-gray-800 dark:text-gray-100 border border-white/80 dark:border-gray-700 shadow-xs">
            Bridge Reward 스타일로 조건·진행률·예상 시상금을 한 눈에
          </span>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="rounded-xl bg-white/70 dark:bg-gray-900/40 border border-white/80 dark:border-violet-800/60 p-3 flex flex-col gap-2">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              <span className="text-[11px] text-gray-600 dark:text-gray-400 truncate">1/19~1/31 + 2/1~2/18</span>
            </div>
            <span
          ...
          <div className="flex justify-between text-[11px] text-gray-600 dark:text-gray-400">
            <span>1월 구간 {formatMan(baseJan)}만</span>
            <span>2월 구간 {formatMan(baseFeb)}만</span>
          </div>

          <div className="h-2.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden border border-gray-200/80 dark:border-gray-700/70">
            <div
          ...
          <p className="text-[11px] text-gray-600 dark:text-gray-400">
            2월 구간 기준 10만 대비 {Math.min(100, Math.round(baseProgress))}% 진행
          </p>

          <div className="mt-1 pt-2 border-t border-gray-200/80 dark:border-gray-700/70 flex items-center justify-between text-xs">
            <span className="text-gray-600 dark:text-gray-300">예상 시상금</span>
        ...
        <div className="rounded-xl bg-white/70 dark:bg-gray-900/40 border border-white/80 dark:border-violet-800/60 p-3 flex flex-col gap-2">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              <span className="text-[11px] text-gray-600 dark:text-gray-400 truncate">1/26~1/31 + 2/1~2/18</span>
            </div>
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${
                extraStatus === "조건 달성"
                  ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-100 dark:border-emerald-700"
                  : extraStatus === "달성 가능성 높음"
                    ? "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/40 dark:text-amber-100 dark:border-amber-700"
                    : "bg-gray-50 text-gray-600 border-gray-200 dark:bg-gray-900/40 dark:text-gray-300 dark:border-gray-700"
              }`}
            >
          ...
          <div className="flex justify-between text-[11px] text-gray-600 dark:text-gray-400">
            <span>1월 추가 구간 {formatMan(extraJan)}만</span>
            <span>2월 구간 {formatMan(extraFeb)}만</span>
          </div>
          ...
          <p className="text-[11px] text-gray-600 dark:text-gray-400">
            2월 구간 기준 10만 대비 {Math.min(100, Math.round(extraProgress))}% 진행
          </p>
...
function ContinuousRun23Card({
  ...
  return (
    <div className="relative rounded-2xl border p-5 flex flex-col gap-4 overflow-hidden bg-gradient-to-br from-violet-50 to-violet-100/80 dark:from-violet-950/40 dark:to-violet-900/30 border-violet-200/80 dark:border-violet-700/60 shadow-lg shadow-violet-200/20 dark:shadow-violet-900/20">
      ...
      <div className="relative grid grid-cols-[1fr_2fr] gap-0 min-h-[220px]">
        {/* 가운데 세로 라인 (좌·우 영역 경계) */}
...
export function Dashboard({
  ...
  // GA별 리다이렉트: direct에서 파트너 선택 시 → /partner, partner에서 비파트너 선택 시 → /direct
  useEffect(() => {
    if (mode === "all" || !selectedAgent) return;
    const branch = (selectedAgent?.branch || "") as string;
    const isPartner = branch.includes("파트너");
    if (mode === "direct" && isPartner) {
      router.replace(`/partner?code=${encodeURIComponent(selectedAgent.code)}`);
      return;
    }
    if (mode === "partner" && !isPartner) {
      router.replace(`/direct?code=${encodeURIComponent(selectedAgent.code)}`);
    }
  }, [mode, selectedAgent, router]);

  // URL의 code로 진입한 경우 agents 로드 후 해당 설계사 선택 (캡처 모드에서도)
  useEffect(() => {
    if (!initialCode || !agents.length) return;
    const found = agents.find((a: any) => String(a.code) === String(initialCode));
  ...
  const showDirectContent = (mode === "all" || mode === "direct") && !isPartnerBranch;
                              ...
                              {toShow.map((agent) => (
                                ...
                                >
                                  ...
                                  <span className="text-xs text-gray-500">
                                    3월 {formatMan(agent.performance?.[searchMonthKey] || 0)}만
                                  </span>
                                </button>
                              ))}
                              {filtered.length > 80 && (
            ...
            <div className="mb-6">
              ...
              {showPartnerContent && (
                <div className="grid grid-cols-1 gap-4 md:gap-5 mb-6 md:grid-cols-4">
                  {selectedViewMonth === 1 && (
                    <>
                      <PartnerPrizeCardFull index={1} title="정규+파트너 추가" badges={["익월", "450%"]} subtext={`1월 실적 ${formatMan(currentMonthPerf)}만`} showTierButtons={false} tierPerf={0} expectedPrize={regularPrize} variant="green" emphasizePrize />
                      <PartnerPrizeCardFull index={2} title="1주차 인보험" badges={["13회차", "100%"]} subtext={`1주차 실적 ${formatMan(viewW1)}만`} showTierButtons tierPerf={viewW1} expectedPrize={p?.productWeek1PrizeJan ?? 0} variant="sky" />
                      <PartnerPrizeCardFull index={3} title="1주차 상품" badges={["13회차", "100%"]} subtext={`1주차 실적 ${formatMan(viewW1)}만`} showTierButtons tierPerf={viewW1} expectedPrize={p?.productWeek1PrizeJan ?? 0} variant="green" />
                      <PartnerPrizeCardFull index={4} title="2주차 인보험" badges={["13회차", "100%"]} subtext={`2주차 실적 ${formatMan(viewW2)}만`} showTierButtons tierPerf={viewW2} expectedPrize={p?.productWeek2PrizeJan ?? 0} variant="sky" />
                      <PartnerPrizeCardFull index={5} title="2주차 상품" badges={["13회차", "100%"]} subtext={`2주차 실적 ${formatMan(viewW2)}만`} showTierButtons tierPerf={viewW2} expectedPrize={p?.productWeek2PrizeJan ?? 0} variant="green" />
                      <PartnerPrizeCardFull index={6} title="12~1월 연속가동" badges={["13회차", "최대300%"]} subtext={`12월 구간 ${formatMan(p?.continuous121Dec ?? 0)}만 · 1월 구간 ${formatMan(p?.continuous121Jan ?? 0)}만`} showTierButtons tierPerf={p?.continuous121Dec ?? 0} tierPerfB={p?.continuous121Jan ?? 0} expectedPrize={p?.continuous121Prize ?? 0} variant="purple" />
                      <PartnerPrizeCardFull index={7} title="3주차 인보험" badges={["13회차", "100%"]} subtext={`3주차 실적 ${formatMan(viewW3)}만`} showTierButtons tierPerf={viewW3} expectedPrize={p?.week3PrizeJan ?? 0} variant="sky" />
                      <PartnerPrizeCardFull index={8} title="4주차 인보험" badges={["13회차", "100%"]} subtext={`4주차 실적 ${formatMan(p?.week4Jan ?? 0)}만`} showTierButtons tierPerf={p?.week4Jan ?? 0} expectedPrize={p?.week4PrizeJan ?? 0} variant="green" />
                      <ContinuousRun12Card
                      ...
                      />
                      <PartnerPrizeCardFull index={11} title="메리츠클럽 플러스" badges={["13회차"]} subtext={`1월 실적 ${formatMan(febPerf)}만 / 목표 ${formatMan(plusTarget || 200000)}만`} showTierButtons={false} tierPerf={0} expectedPrize={meritzClubPlusPrize} variant="yellow" isMCPlus mcPlusCurrent={febPerf} mcPlusTarget={plusTarget || 200000} mcPlusProgress={plusProgress} />
                    </>
                  )}
                  {selectedViewMonth === 2 && (
                    <>
                      <PartnerPrizeCardFull index={1} title="정규+파트너 추가" badges={["익월", "450%"]} subtext={`2월 실적 ${formatMan(currentMonthPerf)}만`} showTierButtons={false} tierPerf={0} expectedPrize={regularPrize} variant="green" emphasizePrize />
                      <PartnerPrizeCardFull index={2} title="1주차 인보험" badges={["13회차", "200%"]} subtext={`1주차 실적 ${formatMan(viewW1)}만`} showTierButtons tierPerf={viewW1} expectedPrize={p?.productWeek1InsPrize ?? p?.productWeek1Prize ?? 0} variant="sky" />
                      <PartnerPrizeCardFull index={3} title="1주차 상품" badges={["13회차", "100%"]} subtext={`1주차 실적 ${formatMan(viewW1)}만`} showTierButtons tierPerf={viewW1} expectedPrize={p?.productWeek1Prize ?? 0} variant="green" />
                      <PartnerPrizeCardFull index={4} title="2주차 인보험" badges={["13회차", "100%"]} subtext={`2주차 실적 ${formatMan(viewW2)}만`} showTierButtons tierPerf={viewW2} expectedPrize={p?.productWeek2InsPrize ?? getPartnerTierPrize(viewW2)} variant="sky" />
                      <PartnerPrizeCardFull index={5} title="2주차 상품" badges={["13회차", "100%"]} subtext={`2주차 실적 ${formatMan(viewW2)}만`} showTierButtons tierPerf={viewW2} expectedPrize={p?.productWeek2Prize ?? getPartnerTierPrize(viewW2)} variant="green" />
                      <PartnerPrizeCardFull index={6} title="1~2월 연속가동 (요약)" badges={["13회차", "최대300%"]} subtext={`1월 구간 ${formatMan(p?.continuous12Jan ?? 0)}만 · 2월 구간 ${formatMan(p?.continuous12Feb ?? 0)}만`} showTierButtons tierPerf={p?.continuous12Jan ?? 0} tierPerfB={p?.continuous12Feb ?? 0} expectedPrize={p?.continuous12Prize ?? 0} variant="purple" />
                      <ContinuousRun12Card
                        baseJan={p?.continuous12Jan ?? 0}
                        baseFeb={p?.continuous12Feb ?? 0}
                        basePrize={p?.continuous12Prize ?? 0}
                        ...
                        extraPrize={p?.continuous12ExtraPrize ?? 0}
                      />
                      <PartnerPrizeCardFull index={8} title="3주차 인보험" badges={["13회차", "100%"]} subtext={`3주차 실적 ${formatMan(viewW3Feb)}만`} showTierButtons tierPerf={viewW3Feb} expectedPrize={p?.week3Prize ?? getPartnerTierPrize(viewW3Feb)} variant="sky" />
                      <PartnerPrizeCardFull index={9} title="3~4주 인보험" badges={["13회차", "100%"]} subtext={`3~4주 실적 ${formatMan(p?.week34Sum ?? 0)}만`} showTierButtons tierPerf={p?.week34Sum ?? 0} expectedPrize={p?.week34Prize ?? getPartnerTierPrize(p?.week34Sum ?? 0)} variant="sky" />
                      ...
                      <PartnerPrizeCardFull index={11} title="메리츠클럽 플러스" badges={["13회차"]} subtext={`2월 실적 ${formatMan(currentMonthPerf)}만 / 목표 ${formatMan(plusTarget || 200000)}만`} showTierButtons={false} tierPerf={0} expectedPrize={meritzClubPlusPrize} variant="yellow" isMCPlus mcPlusCurrent={currentMonthPerf} mcPlusTarget={plusTarget || 200000} mcPlusProgress={plusProgress} />
                    </>
                  )}
                </div>
              )}
