            ...
            <div className="mb-6">
              ...
              {showPartnerContent && (
                <div className="grid grid-cols-1 gap-4 md:gap-5 mb-6 sm:grid-cols-2 lg:grid-cols-6">
                                ...
                                dot={(props: any) => {
                                  ...
                                  return (
                                    <circle
                                      ...
                                      onClick={
                                        monthNum != null ? () => setSelectedViewMonth(monthNum) : undefined
                                      }
                                    />
                                  );
                                }}
                  ...
                  {selectedViewMonth === 3 && (
                    <>
                      {/* 1행: 3월 1주차 인보험·상품 + 2-3월 연속가동 */}
                      <div className="lg:col-span-6 grid grid-cols-1 gap-4 md:gap-5 sm:grid-cols-2 lg:grid-cols-6 items-stretch">
                        ...
                        <div className="lg:col-span-4 h-full flex flex-col">
                          <ContinuousRun23Card
                            febPerf={prevMonthPerf}
                            febExtraPerf={prevMonthPerf}
                            march15Perf={currentMonthPerf}
                            march8Perf={currentMonthPerf}
                            basePrize={p?.continuous23Prize ?? getPartnerContinuousPrize(prevMonthPerf)}
                            extraPrize={p?.continuous23ExtraPrize ?? getPartnerContinuousPrize(prevMonthPerf)}
                          />
                        </div>
                      </div>
                      {/* 2행: 정규+ 파트너 · 2배 메리츠클럽 · 메리츠클럽 플러스 (한 줄에 3장) */}
                      <div className="lg:col-span-2 h-full min-h-0 flex flex-col">
                        <PartnerPrizeCardFull
                          index={1}
                          subtitle="익월 정률시상"
                          title="정규+ 파트너 추가 시상"
                          badges={["익월", "450%"]}
                          subtext={`3월 실적 ${formatMan(currentMonthPerf)}만`}
                          showTierButtons={false}
                          tierPerf={0}
                          expectedPrize={regularPrize}
                          variant="green"
