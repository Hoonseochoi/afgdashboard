...
/** 3월 3주차 특별 현금시상: 10→10, 20→20, 30→30, 50→100, 80→160, 100→300, 120→300 (만원) */
export const MAR_W3_SPECIAL_PRIZES: [number, number][] = [
  [100000, 100000],
  [200000, 200000],
  [300000, 300000],
  [500000, 1000000],
...
/**
 * 3월 3주차 특별 현금시상 (구간 10/20/30/50/80/100/120만)
 */
export const calculateMarW3SpecialPrize = (w3Perf: number): any => {
  return calculateIncentiveTier(w3Perf, MAR_W3_SPECIAL_PRIZES);
};
...
export const calculateIncentiveData = (
  ...
  const week3Perf = weekData.find(w => w.week === 3)?.performance ?? 0;
