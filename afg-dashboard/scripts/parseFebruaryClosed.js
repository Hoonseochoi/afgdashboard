 ...
 *
 * 실행: node scripts/parseFebruaryClosed.js
 */
const XLSX = require("xlsx");
const path = require("path");
const fs = require("fs");

// afg-dashboard/ 폴더에서 실행 (node scripts/parseFebruaryClosed.js)
const ROOT   = path.resolve(__dirname, '..');
const DATA   = path.resolve(ROOT, '..', 'data', 'fix');
const INPUT  = path.join(DATA, '2월마감MC_LIST_OUT_202602.xlsx');
const OUTPUT = path.join(ROOT, 'src', 'data', 'february_closed.json');
...
function parse() {
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    const feb  = Number(row[COL.FEB])   || 0;
  ...
  console.log(`✅ 2월 마감 파싱 완료: ${Object.keys(result).length}명 → ${OUTPUT}`);
